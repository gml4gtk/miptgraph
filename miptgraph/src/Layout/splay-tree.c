...
static inline void *
splay_tree_malloc (size_t n)
{
  return (calloc (1, n));
}

static inline void *
splay_tree_free (void *ptr)
{
  if (ptr)
    {
      free (ptr);
...
splay_tree_delete (splay_tree sp)
{
  ...
  if (sp->root)
      ...
      while (spn)
	{
      ...
      keys =
	(splay_tree_key *) splay_tree_malloc (count *
					      sizeof (splay_tree_key));
...
splay_tree_insert (splay_tree sp, splay_tree_key key, splay_tree_value value)
{
  ...
  spn =
    (splay_tree_node) splay_tree_malloc (sizeof (struct splay_tree_node_n));
